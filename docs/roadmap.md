...
| MER-C11 | 顾客入店码管理 | 收银台/二维码打印助手 | 可生成、更新、分发收银二维码 |
| MER-C03 | 经营看板 | 看板 Tab | 显示营销概览与激活策略概要 |
| MER-C04 | AI 对话与提案 | 营销助手 Tab | 可发起 AI 对话并接收策略提案 |
| MER-C05 | 提案决策 | 营销助手/提案待办 | AI 提案默认自动启用，支持开关与删除 |
| MER-C06 | 策略治理 | 营销助手/五阶段工具 | 可按生命周期阶段启停策略并留痕 |
| MER-C07 | 执行回放 | 审计中心 Tab | 可查看策略执行结果与原因 |
| MER-C08 | 风险与紧急停机 | 风险与实验 Tab | 可控制预算风险并紧急停机 |
| MER-C09 | 自动化运营 | 营销助手/审计中心 | 管理策略激活状态并查看自动执行结果 |
| MER-C10 | KPI 与发布门 | KPI/发布门面板 | 可基于指标做 Go/No-Go 判断 |
| MER-C12 | 消息与提醒机制 | Tab 与模块红点 | 通过红点感知待办、执行结果与风险提醒 |

老板端信息架构口径（当前生效）：
1. 底部导航固定为 `看板`、`营销`、`收银`、`审计`、`风险` 五入口。
2. 不保留“高级工具”二级聚合入口，核心能力均可一跳直达。
3. 不保留独立审批中心与提醒中心页面，待办提醒通过 Tab 与模块红点表达。
...
- 老板视角完成标准：可完成登录、开店，并在首页按三步完成当日基础经营动作。
...
- 老板视角完成标准：可完成策略开关/删除、执行回放审计、风险停机，并通过红点感知关键待办。
...
| PKG-S050-MER-01 | merchant | MER-C05, MER-C06, MER-C07, MER-C08 | 完成老板端策略治理、回放审计、风险控制闭环 | 老板端治理闭环流程 | PKG-S050-SRV-01 | done |
| PKG-S050-MER-02 | merchant | MER-C12 | 建立老板端红点提醒机制（待办/告警） | 老板端提醒流程 | PKG-S050-SRV-02 | done |
...
- 老板视角完成标准：可进行 AI 对话并完成提案治理，AI 提案默认自动启用，`OWNER` 可执行开关与删除。
...
- 老板端界面口径：安卓端顶部与底部区域均完成安全区适配，Tab 页面隐藏系统顶部栏并以全屏内容区展示（首屏不得与系统通知栏重叠）；底部导航统一为 `看板/营销/收银/审计/风险`。
...
- 老板视角完成标准：通过策略激活清单管理生效范围，并在回放中追踪自动执行结果。
- 顾客视角完成标准：自动触达频率可控、体验可解释、订阅偏好可管理。

| PackageID | Lane | CapabilityID | Macro Requirement | Deliverable | DependsOn | Status |
| --- | --- | --- | --- | --- | --- | --- |
| PKG-S100-SRV-01 | server | SRV-C08, SRV-C11 | 建立事件驱动自动化触发、编排与触达频控能力 | 自动化能力基线（执行日志、订阅偏好） | none | done |
| PKG-S100-MER-01 | merchant | MER-C09 | 建立老板端策略激活清单与自动执行回放能力 | 自动化运营流程（无事件开关） | PKG-S100-SRV-01 | done |
...
| RB-NOTIFY-055 | 红点提醒或触达频控缺口 | server + merchant + customer |
