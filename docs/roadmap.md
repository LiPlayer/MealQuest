...
### 01.6 Active / Legacy 双区规则（V14.0 起生效）

1. `Active` 区是当前执行真源；指针、验收、发布判定仅以 Active 区为准。
2. `Legacy` 区仅用于历史追溯、审计与回放，不参与当前指针判定。
3. 当 Active 与 Legacy 口径冲突时，始终以 Active 区为准。
4. Legacy 区禁止增量扩展新需求，只允许修正历史事实错误。

---

## 02. 当前生效主线（Active，V14.0）

> 说明：本区为当前执行真源，承接 `docs/specs/mealquest-spec.md` 的 V14.0 生效口径（五阶段 + 全局价值优化）。

### 02.0 Active 指针判定（唯一来源）

1. `Status = doing` 的 Active Step 即当前执行指针。
2. Active 指针完成后按顺序进入首个 `todo`。
3. Legacy Step 状态变化不影响 Active 指针。

| StepID | Phase | Outcome（结果定义） | Dependency | Status |
| --- | --- | --- | --- | --- |
| A010 | P0 | V14.0 生效口径冻结（spec/roadmap 对齐，五阶段替代六策略） | 无 | doing |
| A020 | P0 | 策略合同升级（`stage/objective/decisionSignals/gameSupport`）可回归 | A010 done | todo |
| A030 | P1 | 建议层按 Global Value 排序且与执行硬门解耦可回归 | A020 done | todo |
| A040 | P1 | 五阶段策略闭环（Acq/Act/Engagement/Expansion/Retention）可回归 | A030 done | todo |
| A050 | P1 | 长期 KPI 与 Go/No-Go 判定可执行 | A040 done | todo |
| A060 | P2 | A/B 与 uplift 预埋能力上线（非 P1 硬门） | A050 done | todo |

### 02.1 Active 覆盖矩阵（完整性基线）

| Active Clause | Requirement（摘要） | Mapped Items | Coverage |
| --- | --- | --- | --- |
| V14-01 | 五阶段为当前策略主线，六策略转 Legacy | A010, A040 | covered |
| V14-02 | 目标函数为长期全局价值（Global Value） | A010, A030 | covered |
| V14-03 | 执行硬门不变（预算/风险/毛利/审批） | A020, A030, A050 | covered |
| V14-04 | Mini-Game 为互动能力底座（非独立阶段） | A010, A040 | covered |
| V14-05 | KPI 主发布门采用长期价值口径 | A050 | covered |
| V14-06 | P1 不将 A/B 与 uplift 作为发布硬门 | A050, A060 | covered |

### 02.2 Active 任务卡（执行合同）

#### A010 - V14.0 生效口径冻结

- Objective：恢复文档完整性并冻结 V14.0 当前口径。
- Done Definition：
1. `spec` 与 `roadmap` 保持完整结构且无关键信息缺失。
2. 五阶段主线/Legacy 归档边界明确。

- Acceptance Commands：
1. `npm run check:encoding`
2. `rg -n "当前生效口径|Legacy|五阶段|Global Value" docs/specs/mealquest-spec.md docs/roadmap.md`

- Triage Key：`RB-DOC-A010`

#### A020 - 策略合同升级回归

- Objective：确保 `stage/objective/decisionSignals/gameSupport` 全链路可回归。
- Done Definition：
1. schema、模板、评分上下文、可见性口径一致。
2. 不破坏审批与执行主链路。
...
- Triage Key：`RB-CONTRACT-A020`

#### A030 - Global Value 建议层

- Objective：建议层采用长期价值排序，执行层保持硬门约束。
- Done Definition：
1. 候选建议稳定排序。
2. 超预算/超风险/超毛利策略不可执行。

- Triage Key：`RB-GVALUE-A030`

#### A040 - 五阶段闭环

- Objective：交付五阶段策略闭环与 Mini-Game 底座协同。
- Done Definition：
1. 五阶段触发->判定->执行->可见->审计链路闭环。
2. 游戏能力异常不阻断支付。

- Triage Key：`RB-LIFECYCLE-A040`

#### A050 - 长期 KPI 发布门

- Objective：固化长期 KPI 口径和 Go/No-Go 判定。
- Done Definition：
1. `CustomerLTV30/MerchantNetProfit30/PlatformProfit30/GlobalValueIndex` 可查询可解释。

- Triage Key：`RB-KPI-A050`

#### A060 - 实验能力预埋（P2）

- Objective：引入 A/B 与 uplift 基础设施（非 P1 硬门）。
- Triage Key：`RB-EXP-A060`

### 02.3 Active 证据账本

| StepID | Test Ref | Runtime Ref | Review Ref | Result | Verified By | Verified At |
| --- | --- | --- | --- | --- | --- | --- |
| A010 | `npm run check:encoding` | `docs/specs/mealquest-spec.md`; `docs/roadmap.md` | 文档完整性审查 | pending | AI/Agent | 2026-03-05 |
| A020 | `cd MealQuestServer && node -r ts-node/register/transpile-only --test test/policyOs.schema.test.ts test/policyOs.constraints.test.ts test/policyOs.http.integration.test.ts` | `MealQuestServer/src/policyos/*` | 合同回归审查 | pass | AI/Agent | 2026-03-05 |

### 02.4 Active 排障索引

| Triage Key | Symptom | First Command | Owner |
| --- | --- | --- | --- |
| RB-DOC-A010 | 文档缺章/结构缺失 | `rg -n "^## " docs/specs/mealquest-spec.md docs/roadmap.md` | product + server |
| RB-CONTRACT-A020 | 策略合同字段校验失败 | `cd MealQuestServer && npm run policyos:validate-templates` | server |
| RB-GVALUE-A030 | 建议排序与解释不一致 | `cd MealQuestServer && node -r ts-node/register/transpile-only --test test/policyOs.http.integration.test.ts` | server |
| RB-LIFECYCLE-A040 | 五阶段触达/可见性异常 | `cd MealQuestServer && npm test` | server + merchant + customer |
| RB-KPI-A050 | 长期 KPI 缺失或口径漂移 | `cd MealQuestServer && npm test` | server + merchant |
| RB-EXP-A060 | 实验分流异常 | `cd MealQuestServer && npm test` | server |

## 03. Legacy 主路线（历史归档，只读）

> 说明：本表按“开发实现顺序”排序，不按 StepID 数字大小排序。

### 03.0 Legacy 指针判定规则（历史口径）
...
| S140 | P1 | Retention 样例策略闭环（`RET_DORMANT_WINBACK_14D_V1`）可回归 | S130 done | done |
| S150 | P1 | Social Viral 样例策略闭环（`SOC_REFER_DUAL_REWARD_FIRST_PAY_V1`）可回归 | S140 done | doing |
...
## 04. Legacy Step 任务卡（主Step + 三端任务，历史归档）
...
| task_id | lane | task | status | output |
| --- | --- | --- | --- | --- |
| S140-SRV-01 | server | 完成 `RET_DORMANT_WINBACK_14D_V1` 沉默识别、触发与执行链路 | done | 样例闭环可回归 |
| S140-MER-01 | merchant | 展示召回命中、回流率与拦截原因 | done | 商户留存视图 |
| S140-CUS-01 | customer | 展示召回触达反馈与结果一致性 | done | 顾客召回反馈 |
...
- Acceptance Commands：
1. `cd MealQuestServer && npm test`
2. `cd MealQuestServer && npm run test:step:s140`
3. `cd MealQuestMerchant && npm run lint && npm run typecheck`
4. `cd meal-quest-customer && npm run test:regression:ui`
...
- Decision Notes（已确认）：
1. `RET_DORMANT_WINBACK_14D_V1` 默认口径冻结为“平衡券包 + `inactiveDays >= 14` + `isNewUser = false`”。
2. 商户端“回流率”口径冻结为 `Hit / (Hit + Blocked)`（24h 决策窗口）。
...
- Test Layering Note（2026-03-05）：默认 `npm test` 继续绑定 `test:step:s050` 作为治理基线；`S110`、`S120`、`S130` 与 `S140` 均提供独立 step suite（`test:step:s110` / `test:step:s120` / `test:step:s130` / `test:step:s140`）用于阶段回归门。
...
| S140 | `cd MealQuestServer && npm run test:step:s140`；`cd MealQuestServer && node -r ts-node/register/transpile-only --test test/policyOs.constraints.test.ts`；`cd MealQuestMerchant && npm run lint && npm run typecheck`；`cd meal-quest-customer && npm run test:regression:ui` | `docs/qa/s140-retention-winback-closure.md` | `MealQuestServer/src/policyos/templates/strategy-templates.v1.json`; `MealQuestServer/src/policyos/policyRegistry.ts`; `MealQuestServer/src/http/routes/preAuthRoutes.ts`; `MealQuestServer/src/http/routes/stateSnapshot.ts`; `MealQuestServer/src/services/merchantService.ts`; `MealQuestServer/test/policyOs.s140.retention.test.ts`; `MealQuestServer/test/policyOs.s140.visibility.http.test.ts`; `MealQuestMerchant/src/services/apiClient.ts`; `MealQuestMerchant/src/domain/merchantEngine.ts`; `MealQuestMerchant/src/context/MerchantContext.tsx`; `MealQuestMerchant/src/screens/DashboardScreen.tsx`; `meal-quest-customer/test/pages/index.test.tsx`; `MealQuestServer/package.json`; `docs/specs/mealquest-spec.md`; `docs/roadmap.md` | pass | AI/Agent | 2026-03-05 |
...
| RB-RET-140 | Retention 样例策略召回异常 | `cd MealQuestServer && npm run test:step:s140` | server + customer |
...
11. 2026-03-05：冻结 S140 Retention 可执行口径（平衡券包、`inactiveDays >= 14`、`isNewUser = false`、14 天频控）并补齐 roadmap/spec 合同。
12. 2026-03-05：完成 `S140` Retention 样例闭环（`RET_DORMANT_WINBACK_14D_V1`）并新增 `test:step:s140` 收口验证，主路线指针推进至 `S150 doing`。
