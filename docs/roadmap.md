...
| MER-C03 | 经营看板 | 首页（任务中心）/完整看板（高级工具） | 可完成日常三步经营并查看经营趋势与商户收益/Uplift 指标 |
| MER-C04 | AI 对话与提案 | 高级工具/Agent 页面 | 可发起 AI 对话并接收策略提案 |
...
| MER-C09 | 自动化运营 | 策略激活清单/执行回放 | 管理策略激活状态并查看自动执行结果 |
| MER-C10 | KPI 与发布门 | KPI/发布门面板 | 可基于指标做 Go/No-Go 判断 |
| MER-C12 | 消息与提醒中心 | 高级工具/提醒中心 | 可接收审批待办、异常告警与系统提醒 |

老板端信息架构口径（当前生效）：
1. 底部导航默认仅保留 `首页`、`收款码`、`高级工具` 三入口。
2. 首页默认承载“今天要做什么”三步主流程：待办处理、建议执行、结果回看。
3. 审批、回放、风控、提醒、完整看板等能力统一收纳到“高级工具”二级入口。
...
- 老板视角完成标准：可进行 AI 对话并完成提案决策，且仅在 `OWNER` 确认后提案才可进入 Policy OS 执行。
...
- 老板端界面口径：安卓端顶部与底部区域均完成安全区适配，Tab 页面隐藏系统顶部栏并以全屏内容区展示（首屏不得与系统通知栏重叠）；底部导航统一为 `首页/收款码/高级工具`，高级能力收纳到工具页。
