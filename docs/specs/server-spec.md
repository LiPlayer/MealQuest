...
11. `WS: STRATEGY_CHAT_SEND_MESSAGE`（`MANAGER/OWNER`，按“新人见面礼”模板与分支生成待确认提案）
...
18. 策略库当前版本仅启用“新人见面礼”模板分支，且仍遵守“无确认不执行”。
...
23. 策略库：仅“新人见面礼”模板可查询并按分支生成提案，确认后进入活动策略
24. 策略启停：`policy status` 改变可立即影响 Policy OS 执行结果
25. 供应商核验：联盟订单核验成功/失败均可返回并落审计
26. 多店连锁：可配置跨店共享钱包并在支付链路生效
27. 顾客自助注销：注销后账号不可再次登录，交易记录匿名保留
28. 顾客账务中心：顾客可查询本人流水与发票，跨用户/跨商户查询拒绝
