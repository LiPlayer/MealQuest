...
3. 所有动态营销触达必须来自服务端 Policy OS/Story JSON，不允许端上“自作主张”发券。
...
3. 活动区：优先展示服务端返回的 `activities`（由 Policy OS 活动实时映射），无远程数据时回退本地兜底活动。
...
8. `homeSnapshot.activities` 必须可承接服务端动态策略投放卡（而非端上写死模板），且当前版本基线仅为“新人见面礼”。

## 4.2 运行模式

1. 线上落地模式：`TARO_APP_SERVER_BASE_URL` 必填，默认走服务端 API。
2. 远程成功快照需本地缓存（按 `storeId+userId` 维度），用于弱网兜底。
3. 远程失败优先回退最近一次本地缓存快照；无缓存时进入显式错误态。
4. 自动向服务端申请 `CUSTOMER` token，无需手工登录。
5. 服务端 `state` 返回活动注入时，顾客端需零配置渲染并保持视觉兜底。

## 4.3 Story/Policy OS 注入约束

1. 活动卡可展示 Policy OS 事件注入内容。
2. 顾客端仅渲染，不做规则决策。
...
3. 卡片交互：展开/收起与焦点切换。
5. 远程模式成功时走 API，失败时优先展示缓存快照，无缓存时报错。
...
| C-04 | AI/Policy OS 动态触达 | 活动区可承接服务端注入策略卡 | 快照字段校验 |
...
8. 当前活动策略基线明确为“新人见面礼”，其余策略待后续版本补齐。
9. 首页远程快照本地缓存能力已接入，弱网时优先展示最近可用资产快照。
10. 账户中心页面已上线，支持钱包总览/流水列表/发票列表展示。
11. 顾客端已支持自助注销闭环（含二次确认、会话清理与启动页回退）。
