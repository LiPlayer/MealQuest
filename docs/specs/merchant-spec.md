...
1. 当前仅展示并支持 `新人见面礼 (Welcome Gift)` 模板。
2. 模板支持两类执行分支（默认奖励 / 渠道奖励）。
3. 商户可基于“新人见面礼模板 + 分支 + 覆盖参数”生成提案，再进入收件箱确认。
4. 提案生成后不得直接执行，必须保留“无确认不执行”。

## 2.4 活动运营中心（Campaign Ops）

1. 支持将活动策略切换为 `ACTIVE/PAUSED/ARCHIVED`。
2. 策略启停必须即时影响 Policy OS 触发结果。
...
1. 该模块当前版本不纳入商户端需求范围（已删除）。
2. 相关操作入口（转赠/红包/请客）不在商户端展示。
...
1. 展示高风险动作审计记录（支付、退款、提案、熔断、Policy OS）。
...
6. 新人见面礼模板查询、提案生成请求参数完整。

## 6.2 UI 层最低要求

1. App 可稳定挂载。
2. 核心文案与模块存在（驾驶舱/收银/Policy OS）。
3. 实时事件流支持“全部/仅异常”筛选与 payload 展开查看。
4. 审计日志支持动作/状态/时间筛选、分页加载与详情展开复制。
5. 新人见面礼模板可生成提案并回流收件箱。
6. 商户端需提供联盟配置可操作入口（非仅 API 存在）。
...
| M-03 | Policy OS 原子引擎 | 事件+条件+预算联合判定 | 触发测试 |
| M-04 | 智能收银 | 抵扣分层可解释 | `smartCashierVerify` 测试 |
| M-05 | 预算红线 | 活动执行需受预算约束 | 预算消耗断言 |
| M-06 | 标准营销策略库 | 新人见面礼模板分支可生成提案并人工确认执行 | 集成测试 |
| M-07 | 多店连锁联盟 | 支持钱包互通配置与用户跨店同步 | 集成测试 |
