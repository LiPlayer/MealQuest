...
### 0. 角色定义升级（全能 Agent）
AI 数字运营官不是“仅聊天机器人”，而是一个覆盖经营全链路的**全能 Agent**执行中枢：

*   **对话交互层**：用自然语言接收老板意图并解释执行结果。
*   **经营执行层**：可调用账本、发票、审计日志、营销策略、Policy OS 等数字能力。
*   **统一任务层**：所有能力通过统一 Agent 任务接口调度，避免“每个模块一个孤立 UI 操作流”。

当前工程阶段允许先以对话为主入口；中长期目标是将更多传统 UI 操作收敛为 Agent 任务能力。
...
*   **临时人工策略 (Manual Override Policy)**：针对突发经营场景，允许商家临时上调或下调**新人见面礼**参数，覆盖常态逻辑。
...
*   **界面注入**：在用户首页动态插入一张“下雨天专属卡片”或“新人见面礼通知卡”。
...
当前版本仅保留并上线 **“新人见面礼 (Welcome Gift)”**，其他策略后续补齐。
...
*   **时效性杠杆 (Temporal Leverage)**：
    *   **以时间换空间 (Time-Cost Tradeoff)**：遵循 **“越紧急，越昂贵”** 原则。
        *   **高紧迫 (Urgent)**：对于短时效拉新窗口（如：开业首周），允许 AI **临时调高** 新人见面礼获客成本上限，以换取极速起量。
        *   **常态化 (Routine)**：对于长周期拉新任务（如：日常门店首绑），严控**平均单客成本**，通过拉长时间跨度摊薄预算。
    *   **自动释放 (TTL Release)**：所有高预算策略必须绑定 **强制结束时间**，过期自动释放被冻结的预算配额，防止隐性成本堆积。
*   **策略多态性 (Polymorphism)**：
    *   **分支 (Branch)**：同一策略的不同执行逻辑（如：默认奖励分支 vs 渠道奖励分支）。
    *   **多实例 (Multi-Instance)**：支持新人见面礼在不同门店或时段并行生效。
...
*   **新人见面礼 (Welcome Gift)**：**门店级首绑奖励**。
    ...
    *   *系统风控*：**同人模型**（同一用户在同一品牌下仅能享受有限次新人见面礼，防止切换门店刷奖）。
