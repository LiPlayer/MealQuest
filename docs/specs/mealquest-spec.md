...
- 界面可用性基线（当前生效）
  ...
  - 老板端 Tab 页面隐藏系统顶部栏（Header），内容区以全屏形态展示并优先保留经营操作区域。
  - 老板端底部导航默认仅保留三入口：`首页`、`收款码`、`高级工具`。
  - 首页默认展示“今天要做什么”任务清单，覆盖待办处理、一键建议执行、结果回看三步主流程。
  - 审批、回放、风控、自动化、提醒、完整看板等高级能力统一收纳至“高级工具”二级入口。
...
S100 阶段服务端需建立“事件驱动自动化基线”，在不新增生日等新字段前提下，复用现有事件信号完成自动化触发、执行日志与订阅偏好能力。
...
- 触发约束（当前生效）：
  ...
  - 自动化关闭时，仅跳过自动化执行，不阻断登录与支付主链路
  - 自动化执行不得绕过预算/风险/频控/审批边界

接口合同（当前生效）：

1. 自动化配置
- `GET /api/policyos/automation/config`
  - 角色：`OWNER / MANAGER`
  - 输出：`enabled`、`rules`、`updatedAt`、`updatedBy`
- `PUT /api/policyos/automation/config`
  - 角色：`OWNER`
  - 输入：`enabled`、`rules`（支持按事件开关）
  - 作用：自动化熔断与规则启停
...
老板端在 S100 阶段需提供“自动化配置 + 执行日志”运营闭环，支撑老板持续调优自动化策略。

- 页面能力（当前生效）：
  - 自动化配置视图：展示全局开关、事件规则开关、更新时间与更新人
  - 执行日志视图：展示命中/阻断/未命中结果与原因码
  - 日志筛选：支持 `event`、`outcome` 过滤并支持刷新

- 角色权限（当前生效）：
  - `OWNER`：可查看并修改自动化配置，可查看执行日志
  - `MANAGER`：可查看自动化配置与执行日志，不可修改配置
  - 其他角色：页面可见但展示权限受限提示，不发起受限接口请求

- 降级要求（当前生效）：
  - 自动化配置接口异常时，仅配置模块降级并支持重试
  - 执行日志接口异常时，仅日志模块降级并支持重试
  - 自动化模块异常不得阻断老板端看板、审批、回放、提醒等既有主路径
...
老板端在 S110 阶段需建立“实验配置 + 灰度收益监控 + 风险可见 + 一键回滚”闭环，采用 Risk 主入口（位于“高级工具”）、Dashboard 摘要可见的形态，不新增独立 Tab。
...
- 交互与降级要求（当前生效）：
  ...
  - 实验模块异常不得阻断老板端看板、审批、回放、提醒、风控等既有主路径。
...
- 商户端：登录、开店、入店二维码管理、经营看板、智能助手（Agent） 提案、审批中心、执行回放、紧急停机、消息提醒
  - 商户端信息架构（当前生效）：底部导航默认仅 `首页/收款码/高级工具` 三入口，高级治理能力通过“高级工具”进入
