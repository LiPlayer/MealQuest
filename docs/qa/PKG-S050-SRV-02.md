...
2. `cd MealQuestServer && node -r ts-node/register/transpile-only test/notification-service.test.ts`
3. `cd MealQuestServer && node -r ts-node/register/transpile-only test/policy-os.s050.governance.http.test.ts`
