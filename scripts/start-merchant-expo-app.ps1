...
$expoDir = Join-Path $repoRoot "MealQuestMerchantExpo"
if (-not (Test-Path $expoDir)) {
    throw "Expo merchant app directory not found: $expoDir"
}

$resolvedEnvFile = Resolve-EnvFilePath -ProjectDir $expoDir
Import-EnvFile -Path $resolvedEnvFile
Write-Host "[merchant-expo] envFile=$resolvedEnvFile" -ForegroundColor Green

if (-not $NoStartServer) {
    $serverScript = Join-Path $PSScriptRoot "start-server.ps1"
    if (Test-Path $serverScript) {
        Write-Host "[merchant-expo] starting local server..." -ForegroundColor Cyan
...
try {
    if (-not $NoInstall) {
        Write-Host "[merchant-expo] npm install" -ForegroundColor Cyan
        npm install
    ...
    if ($NoDevClient) {
        Write-Host "[merchant-expo] expo start" -ForegroundColor Cyan
        npx expo start
        exit $LASTEXITCODE
    }
    if ($Platform -eq "android") {
        Write-Host "[merchant-expo] expo start --dev-client --android" -ForegroundColor Cyan
        npx expo start --dev-client --android
        exit $LASTEXITCODE
    }
    Write-Host "[merchant-expo] expo start --dev-client --ios" -ForegroundColor Cyan
