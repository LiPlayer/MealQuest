...
*   **各司其职 (Role)**：**“老板定战略 (Branch)，AI 定战术 (Parameters)”**。老板决定走什么路线，AI 推荐最佳参数。
...
### 1. 拉新获客 (Acquisition)
*   **新人见面礼 (Welcome Gift)**：扫码首次登录触发。
    *   **分支 A：确定性 (Deterministic)** - **所见即所得**。直接送一张特定 **[入席令]**（如葱油拌面）。
    *   **分支 B：博弈性 (Mystery)** - **盲盒抽奖**。送 **[新手盲盒]**，100% 中奖，利用赌徒心理。
    *   *商家配置*：**资产类型**（赠送对象）、**每日上限**。
    *   *系统风控*：**硬通货熔断**（若送入席令，严格受预算限制；若送盲盒开碎银，则宽松）。
...
*   **LBS 围栏掉落 (Geofence Drop)**：路过自动触发通知。
    *   **分支 A：无感掉落 (Frictionless)** - 奖励 **碎银/优惠券**。路过即得，无门槛。
    *   **分支 B：到店激活 (Validity Lock)** - 奖励 **稀有碎片**。路过仅由通知，资产入账但处于 **“锁定态”**，需**到店核销或消费**时解算位置后激活。
    *   *商家配置*：**围栏半径**、**掉落内容**。
    *   *系统风控*：**疲劳度控制**（7天1次）、**位置验真**（解锁时校验）。
*   **异业联盟 (Cross-Promo)**：
    *   **分支 A：流量互导** - 送 **碎银**。
...
*   **步数换钱 (Walk to Earn)**：
    ...
    *   *系统风控*：**微信运动数据验真**。
*   **环境关怀 (Contextual Drop)**：
    *   **分支 A：慰藉 (Comfort)** - 雨天/降温掉落热食券。
    *   **分支 B：清凉 (Cooling)** - 高温掉落冰饮券。
