...
*   **同盟模式 (Alliance Model)**：
    ...
    *   **资产隔离**：各门店的库存、入席令、流水独立核算。
    *   **会员互通**：顾客在品牌下任意门店的储值余额与会员等级通用（可选配置）。
    *   **一键切换**：老板端 App 支持秒级切换不同门店的经营视图。

### 7. 离线与弱网韧性 (Offline Resilience)
鉴于餐饮环境的复杂性，系统必须具备 **“优雅降级”** 能力：
*   **资产缓存**：顾客的“入席令”与“会员码”缓存在本地，断网状态下仍可亮码供商户扫码（商户端需在线核销）。
...
*   **预算控制架构 (Budget Architecture)**：
    *   **软通货 (Soft Currency) - 碎银**：仅用于活跃度激励，**不消耗** 商户营销预算。风控宽松，旨在让用户“爽”。
    *   **硬通货 (Hard Currency) - 入席令 & 返储值**：前者消耗**菜品库存**，后者消耗**现金负债**，两者均**直接扣减** 营销预算。风控极其严格。
    *   *控制点*：系统的核心在于控制 **“入席令合成率”** 与 **“返现额度”**，而非限制碎银的产出。
...
*   **新人见面礼 (Welcome Gift)**：**门店级首绑奖励**。
    ...
    *   **分支 A：默认奖励 (Default)** - 若无中间人（自然进店），自动弹窗赠送 **[入席令]**（如葱油拌面）。
...
*   **首单破冰 (First Buy)**：针对用户历史订单数为 0 且当前支付。
    ...
    *   **分支 B：全额返储值 (Balance Cashback)** - **延迟满足 (Post-payment)**。支付后 100% 返还至 **[储值卡]**。锁定复购。
    *   *商家配置*：
    ...
    *   *系统风控*：**设备指纹锁**（防刷单）、**同源拦截**。
*   **面对面邀请 (Referral)**：
    *   **分支 A：低门槛 (Low Barrier)** - **扫码即得**。奖励 **小额碎银**（软通货）。无门槛快速裂变。
    *   **分支 B：高价值 (Value Driven)** - **首单返利**。奖励 **现金/稀有碎片**（硬通货）。新客**必须消费**后才发放。
    *   *商家配置*：
        *   **奖励内容 (Reward)**：邀请人/受邀人各得多少碎银或资产。
...
*   **周期会员日 (Asset Multiplier)**：
    *   **分支 A：资产翻倍 (Asset Boom)** - 碎银/碎片 **2.0x 掉落**。不消耗预算（软通货通胀）。
...
*   **步数换钱 (Walk to Earn)**：
    *   **分支 A：兑换 (Exchange)** - **保底逻辑**。1000 步 = 10 碎银。稳健获取。
    *   **分支 B：步数夺宝 (Step Lotto)** - **盲盒逻辑**。消耗 3000 步开启 **[步数宝箱]**。可能开出 10 碎银（亏），也可能 5000 碎银（赚）。
    *   *商家配置*：
        *   **兑换汇率 (Exchange Rate)**：每多少步兑换多少碎银。
...
*   **连续打卡 (Streak Bonus)**：作为“每日任务”嵌入游戏系统。
    *   **分支 A：累计签到 (Cumulative)** - 7天内累计登录5天即送碎银。宽松，防断签挫败感。
    *   **分支 B：连签挑战 (Challenge)** - 连续7天不断签，送 **[稀有碎片]**。硬核，断签重置。
    *   *商家配置*：
...
### 4. 召回留存 (Retention)
*   **生日关怀 (Birthday Gift)**：
    *   **分支 A：资产礼包 (Asset Gift)** - 直接在生日当天向账户注入一张 **[长寿面入席令]**。
...
*   **沉默唤醒 (Recall Injection)**：
    *   **分支 A：损失厌恶** - 通知“碎银即将过期”。成本极低。
    *   **分支 B：无偿赠予** - 注入 **[无门槛入席令]**。成本高，仅针对高价值流失用户。
    *   *商家配置*：
