...
*   **定位**：品牌数字孪生配置与资产库存中心。
*   **功能**：
    *   **视觉配置**：配置入席令的 3D 模型、合成路径及语义名称。
    *   **库存与供应 (Inventory & Supply)**：
        *   **每日库存设定**：为每个入席令 (Gourmet Card) 设置每日可核销上限 (Daily Cap)。
        *   **硬联锁配置**：可选 **"关联 ERP 库存"** (需 API 对接) 或 **"手动每日额度"** (如：今日限量 50 份)。
        *   **自动沽清**：当核销数 $\ge$ 库存数，系统自动停止该资产在 C 端的掉落与合成，防止超卖。
...
*   **交互入口**：在 [Tab 5 管理] 页面的顶部显眼位置，或者 App 标题栏下拉菜单。
*   **功能逻辑**：
