...
1. 本地演练模式：默认使用本地域引擎（无网络依赖）。
2. 远程联调模式：当 `MQ_USE_REMOTE_API=true` 且 `MQ_SERVER_BASE_URL` 配置后，商户端通过服务端 API 执行：
   - 提案确认
   - 熔断开关
   - TCA 触发
   - 审计日志分页查询
   - WebSocket 实时事件订阅（支付/退款/策略变更）
3. 实时事件流支持点击单条事件展开 payload 明细，便于店长排查现场问题。
4. 远程失败自动降级回本地模式，保障演示连续性。

## 5.2 角色权限边界

1. `OWNER`：可确认提案、可熔断、可触发策略、可退款。
2. `MANAGER`：可退款、可触发策略，不可熔断与提案最终确认。
3. `CLERK`：可收银与查看看板，不可退款、提案确认、熔断。
