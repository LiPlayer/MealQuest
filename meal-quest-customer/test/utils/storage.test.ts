...
describe('Storage Utils', () => {
    ...
    it('should cache and load home snapshot by store/user key', () => {
        ...
        storage.setCachedHomeSnapshot('m_store_001', 'u_fixture_001', snapshot);
        expect(Taro.setStorageSync).toHaveBeenCalledWith(
            'mq_home_snapshot:m_store_001:u_fixture_001',
            JSON.stringify(snapshot),
        );

        (Taro.getStorageSync as jest.Mock).mockReturnValue(JSON.stringify(snapshot));
        const loaded = storage.getCachedHomeSnapshot('m_store_001', 'u_fixture_001');
        expect(Taro.getStorageSync).toHaveBeenCalledWith('mq_home_snapshot:m_store_001:u_fixture_001');
    ...
    it('should clear customer session keys', () => {
        storage.clearCustomerSession('m_store_001', 'u_fixture_001');
        expect(Taro.removeStorageSync).toHaveBeenCalledWith('mq_home_snapshot:m_store_001:u_fixture_001');
