        ...
        <View className="px-0 pt-2 pb-6 flex flex-col gap-6">
            ...
            <View className="px-4 flex flex-col gap-4">
                {ACTIVITIES.map((activity) => (
                    <View
                        key={activity.id}
                        className="group relative overflow-hidden active:scale-[0.98] transition-all duration-200 rounded-2xl bg-white border border-gray-100 p-5 flex flex-row items-center gap-5 shadow-sm"
                    >
                        {/* Decorative Background Element */}
                        <View className={`absolute -right-4 -bottom-4 w-24 h-24 ${activity.color} rounded-full opacity-50 blur-2xl group-active:scale-125 transition-transform`} />
                        ...
                        <View className="flex-1 flex flex-col gap-1 relative z-10 text-ellipsis whitespace-nowrap overflow-hidden">
                            <View className="flex flex-row items-center gap-2">
                                ...
                                <View className={`px-1.5 py-0.5 rounded text-[10px] font-black tracking-tighter ${activity.color} ${activity.textColor}`}>
                                    {activity.tag}
                                </View>
                            </View>
