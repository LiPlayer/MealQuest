...
export default function CustomerCardStack({
  ...
  const cardWidthRpx = 750 - 64;
  const cardHeightRpx = Math.floor(cardWidthRpx / CARD_RATIO);
  const focusShiftRpx = cardHeightRpx - HEADER_H;
  ...
  const focusLayout = useMemo(() => {
    ...
    for (let i = 0; i < totalCards; i += 1) {
      if (i > focusIndex) {
        shifts[i] = focusShiftRpx;
      }
    }

    return shifts;
  }, [focusIndex, focusShiftRpx, totalCards]);
  ...
  const stackHeight = useMemo(
    () => Math.max(...cardTops) + cardHeightRpx + 32,
    [cardHeightRpx, cardTops],
  );
      ...
      {cards.map(({ key }, index) => {
        ...
        return (
          ...
          >
            ...
            {key === 'p01' && (
              <P01_SilverCard style={{ width: '100%', height: '100%' }} silver={wallet.silver} />
            )}
            {key === 'p04' && (
              <P04_FragmentCard
                style={{ width: '100%', height: '100%' }}
