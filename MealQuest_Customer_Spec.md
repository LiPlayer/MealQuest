...
*   **交互逻辑 (Single Focus)**：
    ...
    3.  **拖拽预览 (Drag to Peek)**：
        ...
        *   **方向判断**：
            *   堆叠态下，**向上拖动** = 页面滚动（不触发卡片移动）。
            *   堆叠态下，**向下拖动** = 拖动卡片展开。
        *   **跟手性 (Follow)**：在拖动距离 $\leq$ 卡片高度时，执行 1:1 跟手移动。
        *   **非线性阻尼 (Non-linear Damping)**：当距离超过卡片高度后，拖动阻尼显著增大（阻力感增强），模拟物理拉伸的极限感。
        *   **Card N 及所有 index > N 的卡**：跟随上述物理逻辑整体移动。
        *   **Card index < N 的卡**：轻微跟随（阻尼回响）。
        *   **智能提交 (Smart Commit)**：松手时，若拖动距离超过卡片高度 1/2，则**提交**新状态（展开/折叠）；否则**弹簧复原**。
        *   **卡片额头显示高度**：30px（初始堆叠态可见高度）。
