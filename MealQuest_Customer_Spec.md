...
| | **店铺招牌** | **定位**：紧贴顶部导航下方。<br>**内容**：<br>1. 商户 Logo (圆形/方形)。<br>2. 商户名称 (大号加粗)。<br>3. 品牌 Slogan。<br>4. 营业状态标签。<br>**交互**：**动态隐藏**。页面上滑 **30px** 后，店铺招牌快速淡出隐藏，顶部导航继续可见；继续上滑则导航也渐隐，释放最大视口空间给内容区。 |
| **可滚动区域** | **卡片堆叠区** | **定位**：位于店铺招牌下方。<br>**初始状态**：**折叠态**。所有卡片垂直堆叠，仅露出顶部 **48px** 标题区（额头）。<br>**交互**：<br>1. **下滑 (Swipe Down)**：模拟物理钱包，将卡片向下错开 (Fan Out)，展示完整内容。<br>2. **上滑 (Swipe Up)**：将错开的卡片重新折叠回初始状态。<br>3. **持续上滑**：当卡片折叠后，继续上滑将滚动整个页面，展示下方的策略活动区。 |
| | **策略活动区** | 位于卡片堆叠区下方。采用**竖向滑动列表**展示营销卡片与**任务进度引导**。 |
| **底部固定区** | **支付坞** | **水晶支付坞**：采用**毛玻璃质感**的悬浮条。始终位于全局最高层级 (Fixed Bottom)，遮盖滑动列表内容。 |

### 2.2 核心交互：多卡堆叠与物理手势 (Card Stack & Gestures)

> **视觉隐喻**：界面如同手持的一叠扑克牌。Card 3 (最顶层) 覆盖 Card 0 (最底层)。

1.  **堆叠态 (Default Stack)**：
    *   所有卡片垂直堆叠，仅露出 **30px** 额头 (Header)。
    *   **向上拖动**额头 $\rightarrow$ 驱动整个页面滚动 (Page Scroll)。
    *   **向下拖动**额头 $\rightarrow$ 触发**单卡错开** (Peek)。

2.  **点击/拖拽错开 (Peek Focus)**：
    *   **点击**或**下拖**某张卡片 (Card N) 的额头。
    *   **向上拖动 (Drawer Closure)**：
        *   当向上拖动下方任意一张卡片 (Card N, N > FocusIndex) 时，**其所在堆叠组整体上移**。
        *   **抽屉效应 (Block Move)**：Card FocusIndex+1 至 Card Last 作为一个刚性整体跟随手指上移，保持卡片间距不变。
        *   **视觉隐喻**：如同推回抽屉，所有下层卡片协同运作，避免出现 header 互相遮挡或间距拉大的情况。
    *   **智能提交 (Smart Commit)**：
        *   **速度优先**：若下滑速度 > `800px/s` $\rightarrow$ 直接展开 (Snap Open)。
        *   **距离兜底**：若拖动位移 > `1/2` 卡高 $\rightarrow$ 展开。
        *   **否则**：弹簧回弹 (Revert)。
