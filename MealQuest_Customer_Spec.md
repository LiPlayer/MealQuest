...
*   **测试策略**: 采用 **测试金字塔 (Testing Pyramid)** 策略。
    ...
    *   **Tier 3: 端到端与视觉 (E2E & Visual)** [关键路径覆盖]
        ...
        *   **关键用例（已实现）**:
            *   首页卡片堆叠拖拽回归：拖动 `Card N` 下拉后，应进入 `peek Card N`，从而露出 `Card N-1`（防止回归为错误的 `peek N-1`）。
            *   用例位置：`meal-quest-customer/test/e2e/flow.spec.js`。
...
1.  **堆叠态 (Default Stack)**:
    *   所有卡片垂直堆叠，仅露出 **60px** 卡片 (Header)。
    *   **向上拖动**卡片 $\rightarrow$ **驱动整个页面滚动** (Page Scroll)。
    *   **向下拖动**卡片 $\rightarrow$ **触发物理推挤效果** (Physical Push)。


2.  **点击/拖拽 (Focus & Peek)**：
    *   **点击卡片** $\rightarrow$ **展开卡片 (Peek)**：推开上方卡片，露出当前卡片全部内容。再次点击恢复。
    *   **拖拽触发**：
        *   **快速向下滑动 (Fling)**：即使距离较短，放手后也进入 **Peek** 状态。
        *   **慢速拖动**：超过卡片 **50%** 高度放手进入 **Peek** 状态，否则还原。
    *   **物理推挤 (Recursive Push)**：
        *   **向上拖动**：卡片上移若碰到或超过**下层卡片** (Index - 1) 的位置，则带动下层一起上移。
        *   **向下拖动**：卡片下移若碰到或超过**上层卡片** (Index + 1) 的位置，则带动上层一起下移。

3.  **细节打磨 (Micro-Interactions)**：
    *   **延时阻尼**：拖拽距离在**一个卡片高度**内保持 1:1 线性跟随，超过后施加指数级阻力。
    *   **冲突重置**：进入 Peek 状态后，任何新的拖拽手势都会立即清除 Focus 状态并切回物理跟随。
    *   **触感反馈**：卡片“卡入”Peek 状态或触碰边界时提供轻/中震反馈。
    *   **安全区**：卡片底部预留 `240px` 逻辑空间，避让水晶支付坞（由 ActivityArea 间距保证）。
