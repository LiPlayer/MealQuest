...
> **适用场景**：Taro H5 + 小程序双端统一开发，搭配 Tailwind CSS 使用时的关键约束。

#### 0.1.1 rem 冲突与根字号锁定

| 系统 | 行为 | 示例 |
| :--- | :--- | :--- |
| **Taro H5 Runtime** | 动态设置 `html.fontSize`（如 375px 视口 → `37.5px`）以驱动内部 `rem` 缩放 | `1rem = 37.5px` |
| **Tailwind CSS** | 所有工具类底层使用 `rem` 单位 | `text-xl` = `1.25rem`, `w-16` = `4rem` |
| **冲突结果** | Tailwind 尺寸被二次放大 | `text-xl` 预期 20px → 实际 46.8px |

**解决方案**：在 `app.scss` 中锁定 `html { font-size: 16px !important }`，让 Tailwind 回归标准浏览器基准。

> ⚠️ **规则**：全局样式中 **必须** 保留此声明，否则 H5 端所有 Tailwind 尺寸将失控。

#### 0.1.2 designWidth 配置

*   `config/index.ts` 中 `designWidth` **必须设为 375**（与设计分辨率一致）。
*   此值决定 Taro 的 `px → rpx`（小程序）换算比 (`deviceRatio[375] = 2`，即 `1px → 2rpx`)。
*   若误设为 750，代码中的 `px` 值将在 375px 视口上被缩小一半。

#### 0.1.3 H5 容器约束

*   `#app` 设置 `max-width: 480px; margin: 0 auto`，确保：
    *   **手机浏览器** (≤480px)：页面撑满，与小程序体验一致。
    *   **PC 浏览器** (>480px)：页面居中显示，模拟手机比例，开发调试所见即所得。

#### 0.1.4 尺寸单位使用规范

| 场景 | 推荐单位 | 说明 |
| :--- | :--- | :--- |
| Tailwind 工具类 | `rem`（自动） | 无需手动写，使用 `w-16` / `text-xl` 等类名即可 |
| 自定义 SCSS 中的固定布局 | `px` | 不经过 Taro 的 pxtransform 转换的区域，直接用 `px` |
| 需要跨端自适应的自定义值 | `Taro.pxTransform()` | 在 JS 中动态计算时使用 |
| **禁止** | 手动写 `rem` / `rpx` | 避免与 Taro 或 Tailwind 的转换管线冲突 |

#### 0.1.5 跨端一致性检查清单

- [ ] `html { font-size: 16px !important }` 是否存在于 `app.scss`？
- [ ] `designWidth` 是否为 `375`？
- [ ] `#app` 是否设置了 `max-width` 约束？
- [ ] 是否避免了在 SCSS 中手动使用 `rem` / `rpx`？
- [ ] H5 与真机的元素尺寸比例是否目视一致？
...
| 区域 | 元素 | 描述 |
| :--- | :--- | :--- |
| **顶部固定区** | **顶部导航** | **左上头像**：显示用户圆形头像。<br>**右上胶囊**：模拟小程序胶囊按钮。<br>**店铺简化名**：当店铺招牌不可见时，在头像右侧淡入显示 Emoji + 店铺名（单行截断）。 |
...
### 2.3 动态滚动对齐 (Dynamic Scroll Alignment)

为提升浏览效率，首页引入滚动优化机制：

1.  **卡片自动隐藏**：
    *   当用户在首页向上滚动（Scroll Up）且 `scrollTop > 0` 时，卡片堆叠区开始逐渐**降低透明度**并**收缩高度**。
    *   **触发阈值**：滚动距离在 `0 - 150px` 之间完成从显示到完全消失的过渡。
2.  **内容吸引对齐**：
    *   随着卡片区域收缩，下方的**策略活动区**内容会向上“吸引”对齐，最终紧贴在**店铺招牌**下方。
    *   当卡片完全隐藏后，首页转变为经典的“招牌 + 列表”布局，最大化展示活动内容。
3.  **恢复逻辑**：
    *   当用户回到页面顶部（`scrollTop` 趋于 0）时，卡片堆叠区平滑恢复。
