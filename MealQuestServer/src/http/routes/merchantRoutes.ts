...
function createMerchantRoutesHandler({
  ...
  return async function handleMerchantRoutes({ method, url, req, auth, res }) {
    ...
    if (method === "POST" && url.pathname === "/api/merchant/chat/stream") {
      ...
      const { merchantService } = getServicesForMerchant(merchantId);
      const requestStreamModes = Array.isArray(body && body.streamMode)
        ? body.streamMode
        : ["messages", "updates", "custom"];
      const streamModes = [];
      for (const item of requestStreamModes) {
        const mode = String(item || "").trim();
        if (!mode || streamModes.includes(mode)) {
          continue;
        }
        streamModes.push(mode);
      }
      if (streamModes.length === 0) {
        streamModes.push("messages", "updates", "custom");
      }
      ...
      try {
        const result = await merchantService.sendStrategyChatMessage({
          ...
          streamObserver: (eventPayload) => {
            ...
            const mode = String(eventPayload.mode || "").trim();
            if (!mode || !streamModes.includes(mode)) {
              return;
            }
            writeSseEvent(res, "stream", [mode, eventPayload.chunk]);
          },
        });
