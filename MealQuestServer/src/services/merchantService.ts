function createMerchantService(db, options = {}) {
  ...
  async function sendStrategyChatMessage({
    ...
    const normalizeStreamModes = (source) => {
    ...
    const activeStreamModes = normalizeStreamModes(streamMode);
    const emitStreamChunk = (mode, chunk) => {
      const payload = {
        sessionId: session.sessionId,
        streamId,
        protocol: "langchain.agent.stream",
    ...
    const emitProgress = (phase, status = "running", extras = {}) => {
      ...
      emitStreamChunk("custom", {
        phase: String(phase || "UNKNOWN"),
        status: String(status || "running"),
        tokenCount,
        elapsedMs: Math.max(0, Date.now() - streamStartedAtMs),
        at: new Date().toISOString(),
        ...extras,
      });
    };
    ...
    try {
      ...
      while (!next.done) {
        ...
        if (!mode || !activeStreamModes.includes(mode)) {
          next = await gen.next();
          continue;
        }
        if (mode === "messages") {
          const token = typeof event.tokenText === "string" ? event.tokenText : "";
          if (token) {
            tokenCount += 1;
          ...
          emitProgress("AGENT_STEP_UPDATE");
        }
        emitStreamChunk(mode, event.chunk);
        next = await gen.next();
      }
      aiTurn = next.value;
      emitProgress("AGENT_EXECUTION_END", "completed", {
        resultStatus: aiTurn && aiTurn.status ? String(aiTurn.status) : "",
      });
      if (
        aiTurn &&
        aiTurn.status === "CHAT_REPLY" &&
        (!aiTurn.assistantMessage || !String(aiTurn.assistantMessage).trim()) &&
        streamedAssistantText.trim()
      ) {
        aiTurn.assistantMessage = streamedAssistantText;
      }
    } catch (err) {
      emitProgress("AGENT_EXECUTION_END", "failed", {
        error: err && err.message ? String(err.message) : "stream_failed",
      });
      aiTurn = null;
    }
