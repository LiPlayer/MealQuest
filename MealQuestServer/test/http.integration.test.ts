...
test("merchant phone login returns bound status and enforces merchant scope", async () => {
  ...
  try {
    ...
    const contractApply = await postJson(
      ...
      {
        merchantId: "m_store_001",
        companyName: "Scope Bind Co",
        licenseNo: "91310000MA1SCOPEBIND",
    ...
    const ownerLoginCode = getIssuedPhoneCode(app, bindPhone);
    ...
    const requestCodeNoBinding = await postJson(baseUrl, "/api/auth/merchant/request-code", {
      phone: "+8613999990000"
    });
    assert.equal(requestCodeNoBinding.status, 200);
    const noScopeCode = getIssuedPhoneCode(app, "+8613999990000");
    ...
    assert.equal(unscopedLogin.data.status, "ONBOARD_REQUIRED");
    assert.equal(unscopedLogin.data.profile.merchantId, null);
    assert.equal(typeof unscopedLogin.data.onboardingToken, "string");
  } finally {
    await app.stop();
  }
});
...
test("merchant onboarding completion creates store and returns owner session", async () => {
  const app = createAppServer({ persist: false });
  const port = await app.start(0);
  const baseUrl = `http://127.0.0.1:${port}`;
  ...
  try {
    ...
    const onboardingToken = String(phoneLogin.data.onboardingToken || "");
    ...
    assert.ok(completeOnboard.data.token);
    assert.equal(completeOnboard.data.profile.role, "OWNER");
    assert.equal(completeOnboard.data.profile.phone, "+8613800000000");
    const merchantId = String(completeOnboard.data.profile.merchantId || "");
    assert.ok(merchantId);
    assert.equal(completeOnboard.data.merchant.name, "Custom Test Store");
...
test("merchant onboarding is preserved across re-logins with persistence", async () => {
  ...
  try {
    const requestCode = await postJson(base1, "/api/auth/merchant/request-code", { phone: contactPhone });
    assert.equal(requestCode.status, 200);
    const code = getIssuedPhoneCode(app1, contactPhone);
    assert.ok(code);
    const login = await postJson(base1, "/api/auth/merchant/phone-login", {
      phone: contactPhone,
      code
    });
    assert.equal(login.status, 200);
    assert.equal(login.data.status, "ONBOARD_REQUIRED");
    assert.equal(typeof login.data.onboardingToken, "string");
    ...
    const complete = await postJson(
      ...
      { Authorization: `Bearer ${login.data.onboardingToken}` }
    );
    assert.equal(complete.status, 201);
    assert.equal(complete.data.status, "BOUND");
    merchantId = String(complete.data.profile.merchantId || "");
    assert.ok(merchantId);
